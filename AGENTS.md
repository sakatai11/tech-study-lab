...
コードレビューを行うときは、**変更ファイルの領域に対応する `docs/design.md` の章**を読んでから差分を判断する。1500行を超えるため全文は読まない。
...
複数領域にまたがる場合は該当章をすべて読む。判断に迷う場合は章を広めに取る。
...
重要度は **must-fix**（バグ・仕様違反・セキュリティ）／**should-fix**（ガードレール違反・保守性）／**nit**（好みの範疇）で表す。推測ベースの指摘はせず、確認できなかった懸念は「確認事項」として分ける。指摘ゼロならゼロと報告する（水増ししない）。
