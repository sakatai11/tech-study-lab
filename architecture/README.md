...
Issue #164 の比較実験用。設計意図・振る舞いの一次ソースは `docs/design.md`、現在構造の一次ソースはコードと設定である。`graph.json` は再生成できる参照用 snapshot であり、手動編集しない。
...
- 型チェッカー・制御フロー解析は使わない。import edgeは呼び出しの実行証明ではない。shared宣言名は対象内で一意な既存構成を前提とし、汎用的なbarrel/aliasのシンボル解決器ではない。
- API service/DAL の top-level 関数・型は file + name で識別する。名前付きimportの直接呼出し（aliasを含む）と、関数引数・戻り値に明示された単純な `*Deps` 型を辿れる。shadowing、複合型、型推論は解析しない。`query 'getDueCount' 1` で `ReviewDeps` と関連関数を参照できる。
- API routeは `new Hono().get(...).post(...)` の連鎖、app側は名前付きimportしたroute（factory呼び出しを含む）の静的文字列mountを対象とする。ローカルroot `/` のwrapperは透過として扱う。変数に代入したHonoへの後付け登録、動的path、入れ子mount、aliasされたHono、`on`/`all`、条件別登録、public専用healthは対象外。動的pathや非rootローカルmountは検出できた範囲でエラーにする。
