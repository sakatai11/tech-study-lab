...
questions:
  - id: security-xss-01-q1
    ...
    answerIndex: 0
    explanation: XSS は、信頼できない入力値がスクリプトやイベントハンドラなど、実行可能な文脈としてブラウザに解釈されることで起こります。HTMLタグの解釈を許可するコンテキスト（innerHTML など）にサニタイズされていない入力値を直接渡すと、攻撃者が任意のスクリプトを埋め込めるため危険です。暗号化保存やログ記録、URLエンコード送信はそれ自体では実行の条件を作らないため、XSS の成立条件ではありません。ただし、ログや保存した値も後で危険な HTML 挿入をすれば XSS の原因になり得ます。
