...
`apps/web`（Next.js App Router）の画面・ルーティング設計。ビジュアルモックアップ（[`docs/mockups/DevPath Neumorphism.dc.html`](./mockups/DevPath%20Neumorphism.dc.html)・[SP版](./mockups/DevPath%20SP%20Neumorphism.dc.html)）で確定した画面セットを正式スコープとする。Walking Skeleton（§6：セキュリティ > XSS > 教材1本 + 4択3問）は本章のうち `/`・`/learn/...`・`/quiz/...` を貫通させる最小構成であり、本章の画面数と Walking Skeleton の対象範囲は別物である。

### 7.1 ルート構成

| ルート | 役割 | データ経路 | 主導線 |
| --- | --- | --- | --- |
| `/` ダッシュボード | **今日の復習（due）が主役**。学習統計（正答率・学習時間・連続学習日数）・領域別習得状況・最近のアクティビティ・次のレッスン導線を併せ持つ | due 件数・統計値は API（Server loader → `hc`） | 「復習を始める」→ `/review`、「続きから」→ `/learn/...` |
| `/domains` 学習領域一覧 | 4 領域（Security / Frontend / Backend / Architecture）を俯瞰し、領域ごとの習得率・トピック数を表示。コンテンツ未整備の領域は「準備中」表示で非活性 | 領域別集計は API（`GET /domains`） | 領域カード → `/learn/[domain]/[topic]` |
...
- **レイアウト（サイドバー / ボトムタブ）**：PC は左サイドバー（ロゴ＋ダッシュボード／教材／演習／復習（due件数バッジ）／アナリティクス／学習領域／設定）、本文は右側 1 カラム。SP は上部アプリバー（ロゴ＋アバター）＋下部固定タブバー（ホーム／教材／演習／復習（dueバッジ）／分析の5項目）。「演習」「復習」ナビ項目は直前に扱っていたレッスン（未着手なら先頭レッスン）を対象とする簡易ヒューリスティックで遷移先を決定する（MVP は XSS 1本のため実質固定）。**「設定」は将来の公開機能（認証等、§1 スコープ外）向けのプレースホルダで、MVP ではルートを持たない非活性項目**とする。SP のタブバーはスペース都合で 5 項目に絞り、「学習領域（`/domains`）」へは「教材」タブまたはダッシュボードの領域カードから遷移する。
- **ユーザー**：ログイン UI なし。API（Hono）側が固定 `user_id` を権威的に注入する。将来公開時は「固定値を返す関数」を「認証から `user_id` を引く関数」に差し替えるだけで、画面・API 契約は不変。
- **スタイリング**：Tailwind CSS ＋ Neumorphism ベースのビジュアルデザイン。詳細トークン・実装方針は §8.7。
...
├── components/ui/            # 汎用UI（Button / Card / Badge / ProgressBar 等）。Neumorphism トークンを土台に Tailwind で実装（§8.7）
...
### 8.7 スタイリング（Tailwind + Neumorphism デザインシステム）

ビジュアルモックアップ（[`docs/mockups/DevPath Neumorphism.dc.html`](./mockups/DevPath%20Neumorphism.dc.html)・[SP版](./mockups/DevPath%20SP%20Neumorphism.dc.html)）で確定したトンマナを正式なスタイル方針とする。

- **Tailwind CSS を導入**し、スタイルを Tailwind に寄せる。現状の scaffold 残骸（`*.module.css`、`*.disabled` ファイル群）は**撤去対象**として整理する。
- **ビジュアルスタイル＝ Neumorphism（ソフトUI）**。背景と同色のカードに、明暗2方向のシャドウ（暗い影＋明るいハイライト）で立体感を出す。用途別に3段階：
  - `nm`（凸/浮き出し）：カード・ボタン・パネルの基本表面
  - `nm-in`（凹/くぼみ）：アイコンバッジ、選択済み・ロック状態の入力欄
  - `nm-in-sm`（小さな凹・pill 形状）：タグ、プログレスバーの溝、カウンターバッジ
- **カラートークン**（CSS変数として定義し、Tailwind の `theme.extend.colors` / 任意のテーマ切り替えの土台にする）：

  | トークン | 値 | 用途 |
  | --- | --- | --- |
  | `--nm-base` | `#e4e9f0` | 背景・カード地色 |
  | `--nm-dark` | `#d1d7e0` | 影（暗）側 |
  | `--nm-light` | `#ffffff` | 影（明）側 |
  | `--accent` / `--accent-lt` | `#1a73e8` / `#3f8bf5` | アクセント（ブランドブルー）。CTA・進捗バー・選択状態 |
  | 文字色 | `#3b4457`（見出し）/ `#465063`（本文）/ `#7a8699`・`#93a0b5`（補助） | テキスト階調 |
  | セマンティック | 成功 `#1f9d5c` / 警告 `#c77d13` / 危険 `#dc2626` | 正誤判定・due滞留度・誤答率の色分け |

- **タイポグラフィ**：UI・本文は **Inter**（400/500/600/700/800）、ID・パンくず・コードブロックは **JetBrains Mono**（等幅、識別子と地の文を視覚的に区別する）。
- **Tailwind への落とし込み**：Tailwind のユーティリティだけでは2方向シャドウを素直に表現しづらいため、`globals.css` の `@layer components` に `.nm` / `.nm-in` / `.nm-in-sm` を定義し、`components/ui`（Card・Button・Badge・ProgressBar 等）がそれを土台に組む。色・影の強さは CSS 変数（`--nm-base` 等）で一元管理し、Tailwind の任意値記法（`shadow-[...]`）やコンポーネントクラスから参照する。
- **配色・起伏の可変性**：モックアップにはトーン（クール/ニュートラル/ウォーム）・起伏の深さ・アクセントカラーを切り替えるプレビュー用の仕組みがあるが、これはデザイン検討用のプレビュー機能であり、**アプリ本体にテーマ切り替えUIは設けない**。初期値（クール・標準・ブルー）を唯一の配色として固定する。CSS変数で値を持つのは実装の柔軟性のためであり、将来の要望が具体化した時点でテーマ切り替えを検討する。
