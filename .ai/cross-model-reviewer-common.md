...
この共通定義が追加する契約は、`reviewStage`（`discovery` または `verification`）、`architectureMode: knowledge-graph`、`baseBranch: develop-v2`、`effectiveBase`、`graphCoverage: covered | partial | outside | unmatched`、`graphEvidence`、`graphLimitations`、`sourceVerification`、`reviewPolicy` / `externalReviewDecision` / 規則ID / 具体的根拠 / `decisionHead` である。正規化エージェントは`.ai/skills/issue-dev-orchestrate/references/architecture-context.md`も読み、入力不備時は同文書のcanonical error形式を使う。`graphCoverage: pending`はレビュー入力として認めない。`outside`は抽出対象の定義で確認できた場合、`unmatched`は対象領域内だがqueryが一致しない場合に使う。`graphCoverage` が `partial` / `outside` / `unmatched` の場合やquery結果が空の場合も、node / edgeを推測で補わず、その状態とfallbackによる確認結果を保持する。
...
以下のarchitecture項目は説明的placeholderを値として返さず、architecture-context.mdのcanonical YAMLを全サブキーまで完全展開する。
