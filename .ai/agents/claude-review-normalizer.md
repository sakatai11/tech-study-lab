...
共通実行記録の参照先または必要部分を受け取り、baseと`committedRange`の整合を確認する。Graph証跡の不足は参照先から補い、確認できない範囲は報告する。空結果を架空のnode / edgeで補わない。実際のGit差分との照合はオーケストレーターのpreflight結果を前提とし、自分ではGit操作を行わない。
