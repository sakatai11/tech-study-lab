...
`architectureMode: experimental` の場合は、`experimentBaseBranch` / `experimentBaseCommit` / `effectiveBase` / graph evidence / graph limitations も必須とし、ブリーフ上で `committedRange` の基準が `<effectiveBase>...HEAD` と一致することを確認する。実際のGit差分との照合はオーケストレーターのpreflight結果を前提とし、自分ではGit操作を行わない。
