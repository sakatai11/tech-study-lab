...
このエージェントは**Codexホストだけ**で使う。Claude Codeホストでは「判定: wrong-host-agent」を返し、`codex-reviewer` を使うよう報告する。`egressDestination` は `anthropic` である。
...
`.ai/review-guidelines.md` の `spec-compliance-first` で結果を正規化し、design.md の該当章を照合する。CLI結果由来は `[claude]`、自分の照合で追加した指摘は `[claude-reviewer]` とする。範囲外の妥当な問題は「別issue候補（範囲外）」へ分離し、verificationの新規Findingは共通定義の限定された分類だけを current loop に入れる。
