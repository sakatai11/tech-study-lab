...
1. **認証確認**: `coderabbit auth status` を実行する。
   - 認証済みなら手順2へ進む。
   - Sandbox 内で `signed out` の場合は、同じ `coderabbit auth status` だけを正規の承認・権限昇格経路で再実行する。
   - Sandbox 外でも `signed out` なら、レビューを実行せず「判定: auth-required」を返す。オーケストレーターがユーザーに `coderabbit auth login` を促す。
   - Sandbox 外では認証済みなら、Sandbox 内の結果は認証情報が不可視だった偽陰性として扱い、手順2へ進む。
