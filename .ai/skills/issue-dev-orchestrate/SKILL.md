...
- **なぜレビュー方式を実装前に選ぶか**: 別モデルCLIは private な差分を外部サービスへ送信する。送信の有無は、コードが生まれる前にユーザーが決めるべき事項である。
...
4. **レビュー方式を選択する**。実装前に、利用可能なユーザー確認機能で必ず次のどちらかを選んでもらい、選択結果を `<scratchpad>/review-mode-<N>.md` に記録する。
   - **別モデルCLI（推奨）**: フェーズ5でコミット済み差分を、**ホストランタイムとは別の提供元のCLI**へ送信してレビューする。PR作成前に独立レビューが回るため、修正周回が速い。
   - **GitHub App**: PR作成後にGitHub上のCodeRabbit Appレビューを取得する。未コミット差分を外部送信しない。
   - 別モデルCLIを選んだ場合は、**現在のホストランタイムから使用エージェントを決める**。判定を推測せず、`.ai/runtime-compatibility.md`「別モデルCLIレビューのモデル方針」の表に従う。
...
1. **`reviewer`** と、フェーズ0の表で決めた**別モデルレビュアー**（`codex-reviewer` または `claude-reviewer`）を並列起動する。CLIの実行手順・認証確認・権限昇格経路・コマンドフラグ・使用モデル・`docs/design.md` 該当章の照合方法は `.ai/agents/<エージェント名>.md` が単一ソースであり、オーケストレーターは範囲と同意だけを渡す。**ホストと同じ提供元のCLIを別モデルレビュアーとして起動しない。**
2. CLI用ブリーフに必須の項目: `reviewMode: cross-model-cli`、`reviewerAgent: <codex-reviewer|claude-reviewer>`、base `develop`、対象ブランチ、`reviewRange: develop...HEAD`、`externalEgressApproved: true`、**今回のレビュー直前に取得した**ユーザー同意の原文・時刻・対象、実装方針の要約と受け入れ条件（別モデルレビュアーが仕様準拠を検証するために使う）。
3. 別モデルレビュアーが `approve` / `request-changes` **以外**を返した場合（`external-egress-confirmation-required` / `auth-required` / `local-execution-required` / `rate-limited` / `error`）は、一律に**レビュー未取得**として扱う。理由をユーザーへ報告し、境界条件・保守性・テスト十分性を重点確認する2件目の `reviewer` を代替として起動する。**別モデルレビュアーの指摘ゼロや実行失敗をapproveとして扱わない。**
...
1. `reviewer` を2件、役割分担して並列実行する。1件目は**正確性・セキュリティ・テスト十分性**、2件目は**`docs/design.md` 準拠と受け入れ条件の充足**を主担当とし、2件目のブリーフには変更ファイルの領域に対応する design.md の章（`AGENTS.md`「レビュー規約」の章マッピングに従う）を明記する。別モデルCLIは起動しない。
...
**有効なレビュー経路**: フェーズ0で選択した主要レビュー方式が正常完了した場合を原則とする。CLI方式に限り、別モデルレビュアーがレビュー未取得に終わった場合は、本書で定義した2件目の `reviewer` による代替レビューの正常完了も有効とする。GitHub App方式には代替レビュー経路を設けず、Appレビュー未取得のまま完了条件を満たしたと扱わない。
...
**CLI方式の代替経路**: 別モデルレビュアーがレビュー未取得に終わり、代替として起動した2件目の `reviewer` が `approve` / `request-changes` で正常完了した場合は、その fallback reviewer を修正周回用のレビュー境界として保存してよい。ただし別モデルレビュアーの失敗自体を approve として扱ってはならず、未取得の理由は報告する。
...
5. フェーズ5を**この増分だけ**に絞って再実行する。ブリーフの範囲は `reviewRange: <previous-reviewed-head>...HEAD`、base は `<previous-reviewed-head>` とする。
   - **CLI方式**: この現在のコミット範囲を再掲した**新しい**明示同意を実行直前に取得・記録してから起動する。初回または過去の同意記録を再利用してはならない。別モデルレビュアーには `<previous-reviewed-head>` を base として渡し、初回と同じエージェント（`reviewerAgent`）を使う。
...
2. 別モデルCLI方式でまだpushしていない場合は、作業ブランチの push と `develop` 向けPR作成についてユーザー承認を得てから実行する。GitHub App方式ではフェーズ5で作成済みのPRを再作成しない。
   ```bash
...
4. ユーザーに完了報告する: 実装サマリ／初期実装・レビュー周回のコミット履歴／選択されたレビュー方式と使用した別モデルレビュアー名／そのレビュー結果またはCodeRabbit Appの結果（未取得ならその理由）／テスト結果／作業ブランチ名／PR URL（作成した場合）。
