---
name: pr-review-fix
description: PRのレビューコメント・指摘事項を確認し、各指摘の適用可否を判断した上で修正を実装、品質ゲート（typecheck/lint/test）を通してコミット・プッシュし、レビュースレッドへの返信・解決まで一気通貫で行う。CodexではGitHubコネクタを優先する。「PRの指摘に対応して」「レビューコメントを直して」「PRコメントを解消して」などで使用する。
---
...
Codexでは GitHub コネクタを使い、PR・コメント・レビュー スレッドを取得し、返信・解決する。`gh pr-review` 拡張または `gh api` は、コネクタが利用できない場合に限るフォールバックとする。ローカルの修正・コミット・プッシュは Git を使う。
...
- Codexでは、PRメタデータ・コメント・レビュー スレッドをコネクタで取得する。`gh` の認証状態はCLIフォールバックを使う場合だけ確認する。

## フェーズ1: レビュースレッド一覧取得

Codexではコネクタでレビュー スレッド一覧を取得する。

```bash
gh pr-review threads list --pr <N> --repo <OWNER/REPO>
```

スレッドが無ければコネクタで通常コメントを検索する。以下はCLIフォールバック:
...
コネクタから対象コメント・行・投稿者を確認する。以下はCLIフォールバック:
...
Codexではコネクタで対象のインラインコメントへ返信する。以下はCLIフォールバック:
...
指摘を適用しなかった場合は、上記の代わりに理由を明記して返信する。通常コメントもCodexではコネクタで返信する。以下はCLIフォールバック:
...
Codexではコネクタでレビュー スレッドを再取得する。以下はCLIフォールバック:
...
Codexではコネクタでアクティブなスレッドを解決する。以下はCLIフォールバック:
...
Codexではコネクタでスレッド状態を確認し、`git status` で作業ツリーを確認する。以下はCLIフォールバック:
