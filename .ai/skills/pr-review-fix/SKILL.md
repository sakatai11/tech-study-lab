...
`gh auth status` で認証を確認してから、認証済みの `gh pr-review` 拡張または `gh api` を使い、PR・コメント・レビュースレッドを取得・返信・解決する。Codex AppでGitHubコネクタが接続済みの場合は、同等の操作にコネクタを使ってよい。ローカルの修正・コミット・プッシュは Git を使う。
