...
| 権限整合 | Claude Code は `settings.json`、Codex App / CLI は現在のグローバル `config.toml`・sandbox・approval 規則に照らす。GitHub操作は認証済みの `gh` CLI、または Codex App の接続済みGitHubコネクタのいずれかを利用できることを確認し、一方の製品固有設定を他方へ転用しない | 抵触は must-fix、設定漏れは info |
