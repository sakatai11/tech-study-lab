...
タイトル（接頭辞付き）と本文全文を組み立て、**ユーザーに提示して承認を得る**。
...
- 外部リポジトリの issue / PR を参照情報として載せるが、相手側に cross-reference を作らない場合は、完全 URL と `owner/repo#123` 形式を使わない。インラインコードでも抑止できないため、`owner / repo の issue 123` のように区切って記載する。
