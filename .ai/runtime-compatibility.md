...
- PR・Issue・レビュー・コメント・レビュー スレッドの取得／作成／更新／解決は、利用可能な **GitHub コネクタを優先**する。非公開リポジトリも Codex 側の接続済み認可で操作できる。
- `gh auth status` の失敗だけを理由に GitHub 操作を停止しない。コネクタが利用できる場合はそちらで続行する。
- コネクタが未接続・未提供の場合に限り、認証済みの `gh` CLI をフォールバックとして使う。書き込み操作の前に、現在のランタイムで利用可能なコネクタ機能を確認する。
