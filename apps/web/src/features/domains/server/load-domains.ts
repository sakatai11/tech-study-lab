...
import { getTopicRouteParams } from '@/lib/content'
...
export async function loadDomains(): Promise<DomainsViewModel> {
  ...
  const topicRoutes: DomainTopicRoute[] = getTopicRouteParams().flatMap((route) => {
    const result = domainKeySchema.safeParse(route.domain)
    return result.success ? [{ domain: result.data, topic: route.topic }] : []
  })
