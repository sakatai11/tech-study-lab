...
- frontmatter は `packages/shared/src/schema/content.ts` の Zod スキーマに準拠する。問題は frontmatter の `questions[]`（`{ id, type:'mcq', prompt, choices[], answerIndex, explanation }`）に置く。
