...
入力を以下のいずれかに分類する。判断に迷う場合のみ AskUserQuestion で確認する:
...
曖昧な点は**勝手に補完せず**、実装を左右する場合は AskUserQuestion で確認する。
...
タイトル（接頭辞付き）と本文全文を組み立て、**ユーザーに提示して承認を得る**。実装モード（sonnet / codex）の希望もこのとき確認する。
...
ラベルが存在しない場合は先に作成する（`gh label create <名前> --description "<説明>"`）。
